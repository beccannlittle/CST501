...
def dual_gradient_energy(img):
    ...
    for i in range(0, h):
        ...
        energy[i][w - 1] = dxRed[i][w - 1] + \
            dxGreen[i][w - 1] + dxBlue[i][w - 1]
    ...
    for i in range(0, w):
        ...
        energy[h - 1][i] += dyRed[h - 1][i] + \
            dyGreen[h - 1][i] + dyBlue[h - 1][i]
...
def find_seam(img, energy):
    """Return an array of H integers, for each row of the passed
       image return the column of the seam (lowest energy path).
    """
    h = img.shape[0]
    w = img.shape[1]
    shortestPath = [[[[] for _ in range(2)] for _ in xrange(w)]
                    for _ in xrange(h)]
    for j in range(0, w):
        ...
        for i in range(0, h - 1):
            if (j == 0):
                if (shortestPath[i][j][1] < shortestPath[i][j + 1][1]):
                    ...
                    shortestPath[i + 1][j][1] = shortestPath[i][j][1] + \
                        energy[i + 1][j]
                else:
                    shortestPath[i + 1][j][0] = j + 1
                    shortestPath[i + 1][j][1] = shortestPath[i][j + 1][1] + \
                        energy[i + 1][j]
            if (j == w - 1):
...
def main():
    ...
    small1 = [
        [[255.0, 101.0, 51.0], [255.0, 101.0, 153.0], [255.0, 101.0, 255.0]],
    ...
    small2 = [
        [[97.0, 82.0, 107.0], [220.0, 172.0, 141.0], [243.0, 71.0, 205.0],
            [129.0, 173.0, 222.0], [225.0, 40.0, 209.0], [66.0, 109.0, 219.0]],
        [[181.0, 78.0, 68.0], [15.0, 28.0, 216.0], [245.0, 150.0, 150.0],
         [177.0, 100.0, 167.0], [205.0, 205.0, 177.0], [147.0, 58.0, 99.0]],
        [[196.0, 224.0, 21.0], [166.0, 217.0, 190.0], [128.0, 120.0, 162.0],
         [104.0, 59.0, 110.0], [49.0, 148.0, 137.0], [192.0, 101.0, 89.0]],
        [[83.0, 143.0, 103.0], [110.0, 79.0, 247.0], [106.0, 71.0, 174.0],
         [92.0, 240.0, 205.0], [129.0, 56.0, 146.0], [121.0, 111.0, 147.0]],
        [[82.0, 157.0, 137.0], [92.0, 110.0, 129.0], [183.0, 107.0, 80.0],
